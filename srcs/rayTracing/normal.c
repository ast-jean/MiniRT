...
/*   Updated: 2023/06/14 16:26:38 by slavoie          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector3d C, t_vector3d V, double radius)
{
	...
	perpendicular_vec = vector3d_sub(intersection_to_center, \
	vector3d_mult(V, scalar_proj));
	light_to_center = vector3d_sub(point3d_to_vector3d \
	(init_vars()->light->coord), C);
	distance_to_axis = vector3d_length(vector3d_sub(light_to_center, \
	vector3d_mult(V, vector3d_dot(light_to_center, V))));
...
t_vector3d obj_coord, t_shape shape, t_vector3d light_ray)
{
	if (ft_strcmp(shape.id, "sp"))
		...
		return (cylinder_normal(coords, obj_coord,
				vector3d_norm(point3d_to_vector3d(shape.orientation)), \
				to_double(shape.radius)));
	else
